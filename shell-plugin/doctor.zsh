...
# ZSH Doctor - Diagnostic tool for Pi shell environment
...
echo "$(bold "PI ENVIRONMENT DIAGNOSTICS")"
...
# 2. Check if pi is installed and in PATH
print_section "Pi Installation"

# Check if pi is in PATH
if command -v pi &> /dev/null; then
    local pi_path=$(command -v pi)
    
    # Get pi version and extract just the version number
    local pi_version=$(pi --version 2>&1 | head -n1 | awk '{print $2}')
    if [[ -n "$pi_version" ]]; then
        print_result pass "pi: ${pi_version}"
        print_result info "${pi_path}"
    else
        print_result pass "pi: installed"
        print_result info "${pi_path}"
    fi
else
    print_result fail "Pi binary not found in PATH" "Installation: curl -fsSL https://pi.dev | sh"
fi

# 3. Check shell plugin
print_section "Plugin"

# Check if pi plugin is loaded by checking environment variable
if [[ -n "$_PI_PLUGIN_LOADED" ]]; then
    print_result pass "Pi plugin loaded"
else
    print_result fail "Pi plugin not loaded"
    print_result instruction "Add to your ~/.zshrc:"
    print_result code "eval \"\$(pi zsh plugin)\""
    print_result instruction "Or run: pi zsh setup"
fi


# Check plugin loading order in .zshrc
local zshrc_file="${ZDOTDIR:-$HOME}/.zshrc"
if [[ -f "$zshrc_file" ]] && [[ -n "$_PI_PLUGIN_LOADED" ]]; then
    # Extract line numbers for plugin declarations and pi plugin eval
    local plugins_line=$(grep -n "^[[:space:]]*plugins=(" "$zshrc_file" 2>/dev/null | head -n1 | cut -d: -f1)
    local pi_plugin_line=$(grep -n "eval.*pi.*zsh plugin" "$zshrc_file" 2>/dev/null | head -n1 | cut -d: -f1)

    if [[ -n "$plugins_line" ]] && [[ -n "$pi_plugin_line" ]]; then
        if [[ $pi_plugin_line -lt $plugins_line ]]; then
            print_result fail "Plugin loading order incorrect"
            print_result instruction "Pi plugin (line ${pi_plugin_line}) should be loaded AFTER plugins=() (line ${plugins_line})"
            print_result instruction "Move the pi plugin eval statement after the plugins=() array in ~/.zshrc"
        else
            print_result pass "Plugin loading order correct"
        fi
    elif [[ -n "$pi_plugin_line" ]] && [[ -z "$plugins_line" ]]; then
        # Pi plugin found but no plugins=() array - check for individual plugin sources
        ...
        if [[ "$has_other_plugins" == "true" ]]; then
            ...
            print_result info "Ensure pi plugin is sourced AFTER zsh-autosuggestions and zsh-syntax-highlighting"
        fi
    fi
fi

# 4. Check ZSH theme RPROMPT
print_section "PI RIGHT PROMPT"

# Check if pi theme is loaded by checking environment variable
if [[ -n "$_PI_THEME_LOADED" ]]; then
    print_result pass "Pi theme loaded"
elif (( $+functions[p10k] )); then
    print_result info "Powerlevel10k detected (not using Pi theme)"
elif [[ -n "$ZSH_THEME" ]]; then
    print_result warn "Using theme: ${ZSH_THEME}"
    print_result instruction "To use Pi theme, add to ~/.zshrc:"
    print_result code "eval \"\$(pi zsh theme)\""
else
...
# Pi uses its built-in nucleo-picker for interactive selection
...
# Check editor configuration (PI_EDITOR takes precedence over EDITOR)
if [[ -n "$PI_EDITOR" ]]; then
    print_result pass "PI_EDITOR: ${PI_EDITOR}"
    if [[ -n "$EDITOR" ]]; then
        print_result info "EDITOR also set: ${EDITOR} (ignored)"
    fi
elif [[ -n "$EDITOR" ]]; then
    print_result pass "EDITOR: ${EDITOR}"
    print_result info "TIP: Set PI_EDITOR for pi-specific editor"
else
    print_result warn "No editor configured" "export EDITOR=vim or export PI_EDITOR=vim"
fi
...
if [[ "$platform" == "Darwin" ]]; then
    ...
    if [[ "$check_performed" == "false" ]]; then
        ...
        print_result info "Run 'pi zsh keyboard' for detailed keyboard shortcuts"
    fi
...
if [[ -n "$NERD_FONT" ]]; then
    ...
    print_result info "Pi will auto-detect based on terminal capabilities"
fi

# Show actual icons used in Pi theme for manual verification (skip if explicitly disabled)
...
echo "   $(bold "󱙺 PI 33.0k") $(cyan " tonic-1.0")"
    echo ""
    echo "   Pi uses Nerd Fonts to enrich cli experience, can you see all the icons clearly without any overlap?"
